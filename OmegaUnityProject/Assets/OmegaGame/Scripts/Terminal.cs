...
class Terminal : MonoBehaviour
{
    ...
    void addLogEntries(string text, LogType type)
    {
        ...
        foreach (string line in lines)
        {
            if (logEntries.Count >= TotalLines)
            {
                ...
                if (!isOverflowed) isOverflowed = true;
            }
            else if (scroll)
            {
                Offset += 1;
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Home)) { Offset = 0; }
        if (Input.GetKey(KeyCode.PageDown)) { Offset += 3; }
        if (Input.GetKey(KeyCode.PageUp)) { Offset -= 3; }
        if (Input.GetKey(KeyCode.End)) { Offset = MaxOffset; }
    ...
    void OnGUI()
    {
        if (IsVisible)
        {
            ...
            GUI.Box(new Rect(0, 0, Screen.width, Screen.height), "SWI Common: debug/Terminal.cs [F12] "
            ...
            for (n = 0; n < Offset; n++) { entry = entry.Next; }
